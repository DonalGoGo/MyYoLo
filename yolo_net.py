...
class YOLONet(object):
    ...
    def __init__(self, is_training=True):
        ...
        temp1=np.reshape(temp0,(self.boxes_per_cell, self.cell_size, self.cell_size))
        self.offset = np.transpose(temp1, (1, 2, 0))
    ...
    def loss_layer(self, predicts, labels, scope='loss_layer'):
        with tf.variable_scope(scope):
            ...
            pre_box0 = predict_boxes[:, :, :, :, 0]
            pre_box0_offset = pre_box0 + offset
            pre_box1 = predict_boxes[:, :, :, :, 1]
            trans_offset=tf.transpose(offset, (0, 2, 1, 3))


            predict_boxes_tran = tf.stack([(pre_box0_offset) / self.cell_size,
                                           (pre_box1 + trans_offset) / self.cell_size,
            ...
            per_image_class_loss=tf.reduce_sum(tf.square(class_delta), axis=[1, 2, 3])
            class_loss = tf.reduce_mean(per_image_class_loss, name='class_loss') * self.class_scale
